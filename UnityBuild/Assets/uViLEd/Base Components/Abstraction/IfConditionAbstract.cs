...
namespace uViLEd
{
    ...
    {   
        public enum BranchingConditionType
        {
            Equal,
            Less,
            More,
        ...
        public abstract class IfConditionAbstract<T> : LogicComponent, IInputPointParse
        {
            ...
            [Tooltip("input point for transmitting the first value for the subsequent condition check")]
            public INPUT_POINT<T> FirstCompareValue = new INPUT_POINT<T>();
            [Tooltip("input point for transmitting the second  value for the subsequent condition check")]
            public INPUT_POINT<T> SecondCompareValue = new INPUT_POINT<T>();

            [Tooltip("output point without data transmission, called if compare result is true")]
            public OUTPUT_POINT True = new OUTPUT_POINT();
            [Tooltip("output point without data transmission, called if compare result is false")]
            ...
            [ViewInEditor]
            [Tooltip("the type of comparison between two values")]
            public BranchingConditionType Condition;
            ...
            {
                if(Compare(value, ValueForCompare))
                {
                    True.Execute();
                }else
                {
            ...
            private void CompareExternal()
            {
                ...
                if (Compare(_firstCompareValue, _secondCompareValue))
                {
                    True.Execute();
                }
                else
